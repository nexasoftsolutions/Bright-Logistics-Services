    ...
    <div className="flex flex-col w-full bg-background relative overflow-hidden pb-margin-desktop">
      ...
      >
        ...
        <div className="max-w-[1440px] w-full mx-auto relative z-10 flex flex-col items-center gap-6 mt-20">
          <span className="text-label-bold text-secondary-container uppercase tracking-[0.2em] bg-secondary-container/10 px-4 py-2 rounded-full">
            Equipment &amp; Assets
          </span>
          <h1 className="text-headline-xl max-w-4xl text-white">
          ...
          <p className="text-body-lg max-w-2xl mt-4 text-white/90">
      ...
      <SectionContainer className="w-full relative z-20 mt-16">
              ...
              <div key={vehicle.id} className="group bg-surface-container-lowest rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 relative flex flex-col h-full transform hover:-translate-y-1">
                <div className="w-full h-64 bg-surface-variant relative overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                    <h2 className="text-headline-md text-on-primary">{vehicle.name}</h2>
                  ...
                  <div className="space-y-3">
                    {vehicle.tags && (
                      <div className="flex flex-wrap gap-2 mb-2">
                        {vehicle.tags.map((tag, i) => {
                          ...
                          return (
                            <span key={i} className="inline-flex items-center gap-1 text-label-bold text-primary-container bg-surface-container px-2 py-1 rounded-md text-[10px] uppercase">
                    ...
                    {vehicle.capacity && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-label-bold text-on-surface-variant">Capacity</p>
                          <p className="text-body-sm text-on-surface font-semibold">{vehicle.capacity}</p>
                        </div>
                      </div>
                    )}
                    {vehicle.idealFor && (
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-surface-container flex items-center justify-center text-primary-container">
                          {vehicle.iconVariant && IconMap[vehicle.iconVariant] ? React.createElement(IconMap[vehicle.iconVariant], {className:'w-4 h-4'}) : <Layers className="w-4 h-4" />}
                        </div>
                        <div>
                          <p className="text-label-bold text-on-surface-variant">Ideal For</p>
                          <p className="text-body-sm text-on-surface font-semibold">{vehicle.idealFor}</p>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <section className="mt-24 mb-12 w-full">
          <div className="bg-primary-container rounded-2xl p-8 lg:p-12 text-on-primary relative overflow-hidden flex flex-col md:flex-row gap-12 items-center justify-between shadow-xl">
            ...
            <div className="max-w-xl relative z-10 space-y-6">
              <h3 className="text-headline-lg text-on-primary">{fleetStats.title}</h3>
              <p className="text-body-md text-on-primary-fixed-dim">
                ...
                {fleetStats.features.map((feature, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <span className="text-label-bold text-on-primary uppercase">{feature.label}</span>
                    </div>
                  );
                })}
              </div>
              ...
              {fleetStats.metrics.map((metric, i) => (
                <div key={i} className="flex flex-col items-center">
                  <div className="relative w-24 h-24 flex items-center justify-center">
                    ...
                    <span className="text-headline-sm text-on-primary">{metric.value}</span>
                  </div>
                  <span className="text-label-bold text-on-primary-fixed-dim mt-2">{metric.label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <CTABanner 
          headline="Trust Our Fleet With Your Cargo" 
          description="Experience the reliability of a modern, well-maintained logistics fleet." 
