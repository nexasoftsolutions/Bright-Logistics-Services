  ...
  return (
    <div className="flex flex-col w-full bg-surface">
      ...
      <PageHero 
        ...
        titleClassName="text-headline-xl text-on-primary mb-6"
        subtitleClassName="text-body-lg text-inverse-primary max-w-2xl mx-auto"
      ...
      <SectionContainer as="section" className="py-margin-desktop w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-gutter">
          {blogPosts.map(post => (
            <article key={post.id} id={post.id.toString()} className="group relative flex flex-col bg-surface-container rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300">
              <div className="relative h-64 overflow-hidden">
                ...
                />
                {post.category && (
                  <div className="absolute top-4 left-4 bg-primary text-on-primary px-3 py-1 rounded-full text-label-bold uppercase text-[10px] tracking-wider">
                    {post.category}
                  </div>
                )}
              </div>
              <div className="flex flex-col flex-1 p-6 space-y-4">
                ...
                <h2 className="text-headline-md text-on-surface group-hover:text-primary transition-colors">
                  {post.title}
                </h2>
                <p className="text-body-md text-on-surface-variant flex-1 line-clamp-3">
                  {post.excerpt}
                </p>
                <div className="pt-4 border-t border-outline-variant/30 flex items-center justify-between">
                  <span className="text-label-bold text-outline uppercase tracking-wider">Read Full Article</span>
                </div>
              </div>
              <a className="absolute inset-0 z-10" href={`/blog#${post.id}`}>
      ...
      <section className="w-full bg-primary text-on-primary py-margin-desktop mt-12">
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="max-w-xl">
            <h3 className="text-headline-lg mb-4">Never Miss an Update</h3>
          ...
          <div className="w-full md:w-auto flex flex-col sm:flex-row gap-4">
            <input 
              className="px-6 py-4 rounded-full bg-surface/10 border border-surface/20 text-on-primary text-body-md focus:outline-none focus:ring-2 focus:ring-secondary min-w-[300px]" 
              placeholder="Enter your corporate email" 
              type="email" 
            />
            <button className="px-8 py-4 rounded-full bg-secondary text-on-secondary text-label-bold tracking-wider uppercase hover:bg-secondary-container hover:text-on-secondary-container transition-colors">
              Subscribe
            </button>
          </div>
        </div>
      ...
      <CTABanner 
        ...
        description="Partner with industry experts to streamline your logistics operations." 
